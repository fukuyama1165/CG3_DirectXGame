...
void Object3d::LoadTexture()
{
	...
	// WICテクスチャのロード
	result = LoadFromWICFile(L"Resources/jibun.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	assert(SUCCEEDED(result));
...
void Object3d::CreateModel()
{
	...
	//四角形の頂点データ
	VertexPos verticesPointe[] = {
		{{0.0f,0.0f,0.0f}},
	};


	//メンバ変数にコピー
	std::copy(std::begin(verticesPointe), std::end(verticesPointe), vertices);
