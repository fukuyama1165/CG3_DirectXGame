...
//{
//
//	const float rrdWidth = 10.0f;
//	vertices[i].pos.x = (float)rand() / RAND_MAX * rrdWidth - rrdWidth / 2.0f;
//	vertices[i].pos.y = (float)rand() / RAND_MAX * rrdWidth - rrdWidth / 2.0f;
//	vertices[i].pos.z = (float)rand() / RAND_MAX * rrdWidth - rrdWidth / 2.0f;
//
//}
...
void ParticleManager::Update(int BillboardFlag)
{
	HRESULT result;
	...
	//寿命が尽きたパーティクルを全削除
	particles.remove_if([](Particle& x) {return x.frame >= x.numFrame; });
